...
SELECT ChannelNumber,	
	   DisplayName
FROM   CHANNEL
WHERE  DisplayName = 'SYFYP' 
OR     DisplayName = 'SYFYHDP';
...
GO
SELECT	 CONVERT(VARCHAR, StartTime, 108) +
	     '-' + CONVERT(VARCHAR, EndTime, 108) AS "Time",
	     CONVERT(VARCHAR(20), Title) AS "Title",
	     CONVERT(VARCHAR, EndTime - StartTime, 108) AS "Length",
	     CONVERT(VARCHAR(50), Description) AS "Description"
FROM     all_data
WHERE    DisplayName = 'SYFYHDP'
AND      EndTime > '9/12/2017 00:00:00' AND StartTime <= '09/12/2017 23:59:59'
ORDER BY StartTime;
...
SELECT   TOP 100 
	     ...
	     CONVERT(CHAR, StartTime, 8) AS "Start Time",
	     CONVERT(CHAR, EndTime, 8) AS "End Time",
	     CONVERT(CHAR(30),Title) AS "Title"
FROM     all_data	   
WHERE    StartTime IN ('9/10/2017 20:00:00')
...
SELECT   DISTINCT CONVERT(CHAR(20), Title) AS "Title",
	     ...
	     CONVERT(CHAR(12), OriginalAirDate, 107) AS "Original Air Date"
FROM     SHOW
...
FROM      all_data
WHERE     Genre NOT IN ('Sports event', 'Sports non-event')
...
GO
SELECT  DisplayName,
        CONVERT(VARCHAR, StartTime, 107) + ': ' +  CONVERT(VARCHAR, StartTime, 8) +
	   '-' + CONVERT(VARCHAR, EndTime, 8) AS "Time"
FROM    all_data
WHERE   (DisplayName = 'SYFYHDP'
OR      DisplayName = 'SYFYP')
AND     EpisodeName = 'It''s a Good Life';
