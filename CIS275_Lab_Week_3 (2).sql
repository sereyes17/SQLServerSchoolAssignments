...
USE TV
SELECT   TOP 100
         CONVERT(CHAR(10), DisplayName) AS "Channel Name",
	     ChannelNumber AS "Channel Number",
	     CONVERT(CHAR, StartTime, 108) AS "Start Time",
	     CONVERT(CHAR, EndTime, 108) AS "End Time",
	     CONVERT(CHAR(30), Title) AS "Title"
FROM     SCHEDULE 
JOIN     CHANNEL ON ChannelID = FK_ChannelID
JOIN     SHOW ON ShowID = FK_ShowID
WHERE    StartTime = '09/10/2017 20:00:00'
...
GO
SELECT   SD.ChannelNumber AS "Standard Channel",
	     CONVERT(CHAR(10), SD.DisplayName) AS "Standard Name",
	     HD.ChannelNumber AS "HD Channel",
	     CONVERT(CHAR(10), HD.DisplayName) AS "HD Name"
FROM     CHANNEL AS "SD"
JOIN     CHANNEL AS "HD" ON SD.DisplayName +'HD' = HD.DisplayName 
ORDER BY SD.DisplayName;
...
GO
SELECT     SD.ChannelNumber AS "Standard Channel",
           SD.DisplayName AS "Standard Name",
           HD.ChannelNumber AS "HD Channel",
           HD.DisplayName AS "HD Name"               
FROM       CHANNEL SD 
CROSS JOIN CHANNEL HD
WHERE      LEFT(SD.DisplayName, LEN(SD.DisplayName) - 1) + 'HDP' = HD.DisplayName
AND        SD.DisplayName LIKE '%P'
AND        HD.DisplayName LIKE '%HDP'
AND        SD.ChannelNumber <> HD.ChannelNumber
ORDER BY   SD.DisplayName;
...
GO
SELECT    DISTINCT 
	      CONVERT(CHAR(20), Title) AS "Title",
	      EpisodeName AS "Episode",
	      CONVERT(CHAR(50), Description) AS "Description"
FROM      SCHEDULE
FULL JOIN SHOW ON ShowID = FK_ShowID 
...
GO
USE IMDB
SELECT   CONVERT(CHAR(20), originalTitle) AS "Title",
	     CONVERT(CHAR(20), category) AS "Category",
	     CONVERT(CHAR(20), primaryName) AS "Name",
	     ISNULL(CONVERT(CHAR(20), characters), 'N/A') AS "Characters"
...
JOIN     title_genre ON title_directors.tconst = title_genre.tconst
JOIN     title_basics ON title_directors.tconst = title_basics.tconst
...
FROM     title_directors
JOIN     title_ratings ON title_directors.tconst = title_ratings.tconst
JOIN     title_basics ON title_directors.tconst = title_basics.tconst
JOIN     name_basics ON title_directors.nconst = name_basics.nconst
WHERE    primaryName = 'Quentin Tarantino'
AND		 titleType = 'movie'
...
GO
SELECT   TOP 10
         CONVERT(CHAR(30), primaryTitle) AS "Title",
	     averageRating AS "Rating",
...
SELECT           title_principals.tconst AS "ID",
	             CONVERT(CHAR(20), primaryTitle) AS "Title"
FROM             title_principals
JOIN             name_basics ON title_principals.nconst = name_basics.nconst
JOIN             title_basics ON title_principals.tconst = title_basics.tconst
